...
export default function CampusesPage() {
  ...
  async function fetch() {
    const { data, error: err } = await supabase.from('campuses').select('*').order('sort_order')
  ...
  async function handleSubmit(e) {
    ...
    const { error: err } = editTarget
      ? await supabase.from('campuses').update(payload).eq('id', editTarget.id)
      : await supabase.from('campuses').insert({ ...payload, sort_order: items.length + 1, is_active: true })
    if (err) setError(err.message)
  ...
  async function handleDeactivate() {
    const { error: err } = await supabase.from('campuses').update({ is_active: false }).eq('id', deactivateTarget.id)
    if (err) setError(err.message)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditTarget(null); setForm(emptyForm); setFormErrors({}); setShowForm(v => !v) }} aria-label="Toggle add campus form" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-700">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-4 space-y-4 max-w-md">
          <h2 className="text-sm font-medium text-gray-700">{editTarget ? 'Edit Campus' : 'New Campus'}</h2>
          {[['cam-name', 'Name', 'name'], ['cam-short', 'Short Name', 'short_name']].map(([id, label, key]) => (
            <div key={key}>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} aria-label="Save campus" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-50">{saving ? 'Saving…' : 'Save'}</button>
      ...
      {loading ? <LoadingSkeleton rows={3} /> : items.length === 0 ? <EmptyState message="No campuses found." /> : (
      ...
      <ConfirmDialog open={!!deactivateTarget} title="Deactivate Campus" message={`Deactivate "${deactivateTarget?.name}"?`} onConfirm={handleDeactivate} onCancel={() => setDeactivateTarget(null)} />
    </div>
  )
}
